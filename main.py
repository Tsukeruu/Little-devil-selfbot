try:
    ...
    *Made by Astraa#6100* **|** ||https://github.com/astraadev||\"""
    ...
    @bot.command()
    async def raid(ctx, count: int, delay: float, channel: commands.TextChannelConverter=None, *, message: str):
        await ctx.message.add_reaction('✅')
        await ctx.message.delete() 
        print(f'{ctx.message.author.name} sent the command: {ctx.message.content}')
        guild = ctx.guild
